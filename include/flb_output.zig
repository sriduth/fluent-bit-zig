pub const struct_flb_api = extern struct {
    output_get_property: ?extern fn ([*c]u8, ?*c_void) [*c]u8,
};
pub const struct_flb_plugin_proxy_context = extern struct {
    remote_context: ?*c_void,
};
pub const struct_flb_output_instance = @OpaqueType();
pub const struct_flbgo_output_plugin = extern struct {
    _: ?*c_void,
    api: [*c]struct_flb_api,
    o_ins: ?*struct_flb_output_instance,
    context: [*c]struct_flb_plugin_proxy_context,
};
pub fn output_get_property(key: [*c]u8, plugin: ?*c_void) [*c]u8 {
    var p: [*c]struct_flbgo_output_plugin = @ptrCast([*c]struct_flbgo_output_plugin, @alignCast(@alignOf(struct_flbgo_output_plugin), plugin));
    return p.?.api.?.output_get_property.?(key, @ptrCast(?*c_void, p.?.o_ins));
}
...
pub const flb_api = struct_flb_api;
pub const flb_plugin_proxy_context = struct_flb_plugin_proxy_context;
pub const flb_output_instance = struct_flb_output_instance;
pub const flbgo_output_plugin = struct_flbgo_output_plugin;
